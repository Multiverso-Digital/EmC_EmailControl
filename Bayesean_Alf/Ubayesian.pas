...
uses Classes, StrUtils, SysUtils, DBClient, DB ,Forms,
     generics.collections, generics.defaults, ABSMain;
...
var
  rToken:TToken;
begin
{  if TokenList.active then TokenList.Close;
  TokenList.Params[0].AsString := STR;
  TokenList.open;
}

  //if TokenList.RecordCount > 0 then
  if self.TokenList.Locate(self.campoStr,str,[]) then
  begin
...
begin
  ...
  Except
    result:=false;
    raise Exception.CreateFmt('Erro na criação da Tabela de Tokens',[]);
  end;}
  Result:=true;
end;
...
begin
  ...
  Except
    result:=false;
    raise Exception.CreateFmt('Erro na criação da Tabela Bayesian',[]);
  end; }
  Result:=true;
...
procedure TToken.grava(TokenTable: TClientDataSet; campoStr, CampoBad,
  campoGood: String);
begin
  ...
  if TokenTable.Locate(campoStr,self.strToken,[]) then
  begin
