...
function PegaReceivedFrom(cabec:String):String;
...
for I:= 0 to Lista.Count - 1 do
  begin
    achou := pos('Received: from ',Lista.Strings[i]);
    if achou >  0 then
       begin
        monta := ExtrairRegexp('\d{1,3}(\.\d{1,3}){3}', Lista.Strings[i]);
        if length(monta) > 0  then
              begin
                if (copy(monta,1,2) <> '10') and (copy(monta,1,3) <> '192')
                   and (copy(monta,1,3) <> '127') and  (copy(monta,1,2) <> '1.')
                   and (copy(monta,1,3) <> '172') then
