...
TPOP = class(TidPOP3)
   ...
   UIDOKLida    : THashedStringList;
   UIDNumLida    : THashedStringList;
   Ticket   : Integer;
   Constructor Create;
...
uses Funcoes, UDMSERVER, UDM_Baysean, Umain;

{ TPOP }

constructor TPOP.create();
begin
  Inherited create(Fmain);
  Self.UIDS  := THashedStringList.Create;
  Self.UIDOK := THashedStringList.create;
  Self.UIDNum:= THashedStringList.create;
  Self.UIDOKLida := THashedStringList.create;
  Self.UIDNumLida := THashedStringList.create;
end;
...
{
try
Monta :=  datetimetostr(self.Date);
  except
           self.FiltroResultado.Add(' Message Date: does have a valid Date or missing. '+monta);
           self.Filtrada := 145;
           self.Selecionada := 1;
        end;
 }
...
//    Self.IPSender := ExtrairRegexp(dm_server.ExtraiIP, self.ReceivedFrom);
    ...
    Self.IPSender := PegaReceivedFrom(self.Headers.Text);
